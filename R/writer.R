...
wrSVfix <- function() {
  glue::glue('### Useful stuff \n',
             ...
             '      xo <- diff(range(ggData3$X)) / 200 \n',
             '      yo <- diff(range(ggData3$Y)) / 200 \n',
             '      for(i in theta) {{ \n',
             '        ggData3$X2 = ggData3$X + (cos(i)*xo) \n',
             '        ggData3$Y2 = ggData3$Y + (sin(i)*yo) \n',
             '        ggOut = ggOut + \n',
             '          geom_text(data = ggData3, aes(X2, Y2, label = val), \n',
             '                    color = "grey95", size = lList[inpfsz]) \n',
             '      }} \n',
             '      ggOut = ggOut +  \n',
             '        geom_text(data = ggData3, aes(X, Y, label = val), \n',
             '                  color = "grey10", size = lList[inpfsz]) \n',
             '    }} \n',
             '  }} \n',
             '  if(inpasp == "Square") {{ \n',
...
wrSVmain <- function(prefix) {
  glue::glue('optCrt="{{ option_create: function(data,escape) {{return(\'<div class=\\"create\\"><strong>\' + \'</strong></div>\');}} }}" \n', 
             '  updateSelectizeInput(session, "{prefix}a1inp2", choices = sort(names({prefix}gene)), server = TRUE, \n',
             '                       selected = {prefix}def$gene1, options = list( \n',
             '                         maxOptions = 6, create = TRUE, persist = TRUE, render = I(optCrt))) \n',
             '  updateSelectizeInput(session, "{prefix}a3inp1", choices = sort(names({prefix}gene)), server = TRUE, \n',
             '                       selected = {prefix}def$gene1, options = list( \n',
             '                         maxOptions = 6, create = TRUE, persist = TRUE, render = I(optCrt))) \n',
             '  updateSelectizeInput(session, "{prefix}a3inp2", choices = sort(names({prefix}gene)), server = TRUE, \n',
             '                       selected = {prefix}def$gene2, options = list( \n',
             '                         maxOptions = 6, create = TRUE, persist = TRUE, render = I(optCrt))) \n',
             '  updateSelectizeInput(session, "{prefix}b2inp1", choices = sort(names({prefix}gene)), server = TRUE, \n',
             '                       selected = {prefix}def$gene1, options = list( \n',
             '                         maxOptions = 6, create = TRUE, persist = TRUE, render = I(optCrt))) \n',
             '  updateSelectizeInput(session, "{prefix}b2inp2", choices = sort(names({prefix}gene)), server = TRUE, \n',
             '                       selected = {prefix}def$gene2, options = list( \n',
             '                         maxOptions = 6, create = TRUE, persist = TRUE, render = I(optCrt))) \n',
